...
Categories exist in both upper and lower case versions, so let's make them all lower case.

Several fluid names apper in upper and mixed case versions.  Let's use lower case.

Only 2 of the 589 labels have case variations, so let's leave those alone for now.

```{r}
dimLabItems                                   %>%
  mutate(category = str_to_lower(category),
         fluid    = str_to_lower(fluid))      %>% 
  summarize(n  = n(),
...
factLabEvents                                 %>%
  ...
  mutate(category = str_to_lower(category),
         fluid    = str_to_lower(fluid))      %>% 
  group_by(category, fluid)                   %>%
  summarize(n      = n(),                     
            nLOINC = n_distinct(loinc_code),
            nLabel = n_distinct(label))       %>%
 ungroup()                                    %>%
 arrange(category, fluid)                     %>%
 collect()                                    %>%
 Show()
```
...
```{r}
targetItemID <- 50964    # Osmolality, Measured
```
...
ggplot(targetLab, aes(x = valuenum, y = ..density..))              +
    geom_histogram(fill="cornsilk", color="grey80")                +
    geom_density()                                                 +
    xlim(0, quantile(targetLab$valuenum, 1.00, na.rm=TRUE))        +
    labs(title=paste0("Lab Event: ", targetLab$label),
